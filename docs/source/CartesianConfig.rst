...
variants. The cartesian configuration parser creates a cartesian product of
variants from variant sets; the number of combinations is the multiplication
of the number of variants in each variant set, each combination is a tuple
and the number of elements in this tuple is equal to the number of variant sets.
Eg. if variant sets A and B have 3 and 4 variants respectively, their cartesian
product will have 3 x 4 = 12 tuples of 2 elements.
...
must be indented further right than the ‘variants:’ statement.  When there are
two variants blocks, the output will be all possible combinations of both
variant contents. For example::
...
While combining, the parser forms names for each combination based on
prepending each variant onto a list. In other words, the first variant
name parsed will appear as the right most name component. For example, running
